...
const Dashboard = () => {
  ...
  // Expense form
  const [newExpense, setNewExpense] = useState({ description: '', amount: '', category: 'Outros', is_outing: false, is_credit: false });
  ...
  // --- Fetch user profile, partner, and budget ---
  useEffect(() => {
    if (!user) return;
    ...
    const fetchProfile = async () => {
      ...
      try {
        ...
        // Set saved budget ideally from the user, but fallback to partner if the user doesn't have it set
        ...
        if (data?.partner_id) {
          ...
          setPartnerProfile(partner || { id: data.partner_id, display_name: null });
        }
  ...
  const fetchTransactions = async () => {
    ...
    try {
      ...
      // Totals logic: My total, Couple total, and the special Outing (Saídas) total for the budget
  ...
  const handleAddExpense = async (e) => {
    ...
    try {
      ...
      if (data) {
        ...
        fetch(`${BACKEND_URL}/api/webhooks/open-finance`, {
          method: 'POST',
  ...
  const handleDeleteExpense = async (id, amount, ownerId) => {
    ...
    // Note: To be perfectly accurate, we should delete from budgetUsed if is_outing is true,
    // but without querying it or storing is_outing on state, fetching everything again is safer.
    fetchTransactions();
  };
  ...
  const handleSavePartner = async () => {
    ...
    try {
      ...
      const p = partner || { id: partnerInput.trim(), display_name: null };
      setPartnerProfile(p);
      setPartnerId(p.id);
      setPartnerInput('');
    } catch (err) {
      alert('Erro ao vincular parceiro: ' + err.message);
    } finally {
      setIsSubmitting(false);
    }
  };
  ...
  const handleSaveBudget = async () => {
    ...
    // Also sync to partner so both see the same couple limit
    if (partnerId) {
      await supabase
        .from('profiles')
  ...
  const handleImportCSV = async (e) => {
    ...
    try {
      ...
      setImportResult({ bankName: '?', count: 0, error: err.message });
    } finally {
      setImporting(false);
      e.target.value = '';
    }
